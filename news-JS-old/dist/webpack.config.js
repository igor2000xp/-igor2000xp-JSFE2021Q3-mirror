...
const getEntryPoints = (pages) => pages.reduce((entry, { name, dir, script, style }) => Object.assign(entry, script ? { [name]: makePath(path.join(dir, script)) } : {}, style ? { [`${name}-styles`]: makePath(path.join(dir, style)) } : {}), {});
...
module.exports = ({ development, dirname, isUseTs }) => {
    ...
    return Object.assign({ mode: development ? 'development' : 'production', devtool: development ? 'inline-source-map' : false, entry: getEntryPoints(pages), context: srcPath, output: {
            filename: 'js/[name].[contenthash].js',
            path: path.resolve(dirname, 'dist'),
            assetModuleFilename: '[file]',
        }, target: ['web', 'es6'], module: {
            rules: [
